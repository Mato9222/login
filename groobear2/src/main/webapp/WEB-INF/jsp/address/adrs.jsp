            ...
            <div class="recipients">
                ...
                <div class="row card-body pb-4 d-flex Sflex-wrap align-items-center">
					...
					<div class="row card-body pb-4 d-flex flex-wrap align-items-center ccGroup">
                    	<h3> 참조</h3>
	                    <ul class="non-click ccValue" id="ccValue"> </ul>
		...
		window.addEventListener("message", (event) => {
	        ...
	        if (emplHidden && emplValue) {
	            let empl = `
	                ...
	                let recList = document.querySelector(".recValue");
	                recList.innerHTML += empl; // 기존 내용에 새로운 내용을 추가
	        ...
	        if (ccHidden && ccValue) {
	            let ccEmpl = `
	                <li class='user' id='\${ccHidden}'>
	                    ...
	                    <input type="hidden" class="ccHidden" value="\${ccHidden}"/> 
	        			<button class="remove-btn"> x </button>
...
// 		        		</i>	                    
	                ...
	                let ccList = document.querySelector(".ccValue");
	                ccList.innerHTML += ccEmpl; // 기존 내용에 새로운 내용을 추가
...
// 	            document.querySelector(".ccValue").value = ccEmpl;
	...
	/* 
		$('.send').on("click", function () {
		    function collectValues(groupClass, itemSelector, hiddenSelector, outputId, hiddenId) {
		        const items = [];
		        const hiddenItems = [];
		        
		        $(`.${groupClass} ${itemSelector}`).each(function () {
		            items.push($(this).text());
		        });

		        $(`.${groupClass} ${hiddenSelector}`).each(function () {
		            hiddenItems.push($(this).val());
		        });

		        opener.document.getElementById(outputId).value = items.join();
		        opener.document.getElementById(hiddenId).value = hiddenItems.join();
		    }

		    // 받는 사람 (recGroup)
		    collectValues("recGroup", ".userItem", ".recHidden", "pRec", "emplHidden");

		    // 참조 (ccGroup)
		    collectValues("ccGroup", ".userItem", ".recHidden", "pCc", "ccHidden");
	   				...
	   				res.forEach(emplList => {
	   					...
	   					emplHtml += `
   							<div class="form-check form-check-sm form-check-custom form-check-solid mt-3 emplSel">
		   						<input class="form-check-input" id="emplValue" name="chk" type="checkbox" value="\${emplAtt}"> \${emplValue}
